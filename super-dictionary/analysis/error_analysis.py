...
def detect_section_header(text: str, char_pos: int) -> str:
    ...
    # Sort by position, find the last header before char_pos
...
def analyze_errors(
    ...
    *,
    ...
    # Collect all errors
    false_negatives = []  # Gold spans with no matching prediction
    false_positives = []  # Predicted spans with no matching gold
    wrong_instances = []  # Right concept, wrong location
    boundary_errors = []  # Right location, wrong boundaries
    correct_matches = []  # Good matches
    ...
    for note_id in note_ids:
        ...
        # Track which gold/pred rows have been matched
        gold_matched = set()
        pred_matched = set()

        # For each gold annotation, find the best matching prediction
        for g_idx, gold_row in note_gold.iterrows():
            ...
            # Find predictions with same concept_id
            same_concept = note_pred[note_pred["concept_id"] == g_concept]

            if len(same_concept) == 0:
                # No prediction for this concept at all in this note
            ...
            # Find best IoU match among same-concept predictions
            best_iou = 0.0
            best_p_idx = None
            best_p_row = None
            ...
            if best_iou >= 0.95:
                ...
                # Boundary error - right area, wrong boundaries
                gold_matched.add(g_idx)
                pred_matched.add(best_p_idx)
                ...
                # Some overlap but poor - likely boundary error with low IoU
                gold_matched.add(g_idx)
                pred_matched.add(best_p_idx)
                p_start = int(best_p_row["start"])
                p_end = int(best_p_row["end"])
                ...
                # No overlap at all - check if there's any prediction for this concept
                # Find nearest same-concept prediction
        ...
        for p_idx, pred_row in note_pred.iterrows():
            ...
            # Check if this concept exists in gold at all for this note
...
def main():
    ...
    gold_path = REPO_ROOT / "data" / "old-challenge-split" / "test_annotations.csv"
    notes_path = REPO_ROOT / "data" / "old-challenge-split" / "test_notes.csv"

    print(f"Predictions: {pred_path}")
    print(f"Gold:        {gold_path}")
    print(f"Notes:       {notes_path}")

    results = analyze_errors(str(pred_path), str(gold_path), str(notes_path))
    print_summary(results)
    print_examples(results, n=8)

    # Save detailed CSVs
    out_dir = REPO_ROOT / "outputs" / "old_challenge_split" / "error_analysis"
