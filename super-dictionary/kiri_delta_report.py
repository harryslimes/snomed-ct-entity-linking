#!/usr/bin/env python3
"""Analyze where super-dictionary KIRI helps/hurts vs default KIRI.

Generates per-note and per-concept delta reports, plus character-level
flip analysis showing which concepts gained or lost correct predictions.
"""
...
from runtime_scoring import class_char_iou
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--default-pred", type=Path, required=True,
                        help="Path to default KIRI predictions CSV")
    parser.add_argument("--super-pred", type=Path, required=True,
                        help="Path to super KIRI predictions CSV")
    parser.add_argument("--gold", type=Path, required=True,
                        help="Path to gold annotations CSV")
    parser.add_argument("--default-class-iou", type=Path, required=True,
                        help="Path to default KIRI class IoU CSV")
    parser.add_argument("--super-class-iou", type=Path, required=True,
                        help="Path to super KIRI class IoU CSV")
    parser.add_argument("--concept-names", type=Path, required=True,
                        help="CSV with columns concept_id, concept_name (for readable reports).")
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/delta_report"),
                        help="Output directory for delta reports")
    parser.add_argument(
        "--note-id-source",
        choices=["default", "super", "gold"],
        default="default",
        help="Which file determines the evaluated note_id set.",
    )
    ...
    print(
        f"Mean note-IoU: default={note_rep['iou_default'].mean():0.4f} super={note_rep['iou_super'].mean():0.4f} (delta={note_rep['delta'].mean():+0.4f})"
    )
    print()
